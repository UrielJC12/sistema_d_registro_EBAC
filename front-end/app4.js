...
function agregar_n_alumno() {
    ...
    registro.push({
        ...
        email: email,
        activo: false // Inicializamos en false hasta que se ingrese el número de teléfono
    });
...
function consultar_registros() {
    if (registro.length === 0) {
        ...
        for (var i = 0; i < registro.length; i++) {
            ...
            console.log("Activo: " + registro[i].activo); // Imprimir si está activo
        }
...
function num_telefonico() {
    while (true) {
        ...
        if (activo === "S") {
            ...
            var ultimoAlumno = registro[registro.length - 1];
            ultimoAlumno.num = num; // Añadir el número al último registro (el alumno recién agregado)
            ultimoAlumno.activo = true; // Cambiar el estado a 'activo' (true)
...
function imprimir_edad_por_nombre() {
    var nombre = prompt("Ingrese el nombre del alumno cuya edad desea ver:");
    var alumno = registro.find(alumno => alumno.nombre === nombre);

    if (alumno) {
        console.log("La edad de " + nombre + " es: " + alumno.edad);
    } else {
        console.log("No se encontró a ningún alumno con ese nombre.");
    }
}
// Lógica principal del programa
do {
    var opcion = prompt("Seleccione una opción: \n1. Agregar registro \n2. Consultar registros de alumnos \n3. Eliminar registro \n4. Editar datos del alumno \n5. Imprimir solo tu edad \n6. Salir");
